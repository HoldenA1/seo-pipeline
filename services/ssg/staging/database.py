...
def connect_with_connector(db_user: str, db_pass: str, db_name: str, instance_connection_name: str, connector: Connector) -> Engine:
    """
...
# create the engine
engine = connector = None
if os.environ.get("INSTANCE_CONNECTION_NAME"):
    # initialize Cloud SQL Python Connector object
    connector = Connector()
    engine = connect_with_connector(DB_USER, DB_PASS, DB_NAME, os.getenv("INSTANCE_CONNECTION_NAME"), connector)
else:
...
def get_cities() -> list[tuple]:
    """Returns a list of cities (stored as city, state tuples) for which we have articles written"""
    query = text("SELECT DISTINCT city, state FROM places WHERE status = :status")
    params = { "status": ArticleStatus.PUBLISHED.value }
    try:
        with engine.connect() as conn:
            result = conn.execute(query, params).all()
            return result
    except SQLAlchemyError as e:
        print(f"Database error: {e}")

def get_places_by_location(city: str, state: str) -> list[str]:
    """Returns a list of place_ids (published) in the given city"""
    query = text("SELECT id FROM places WHERE status = :status AND city = :city AND state = :state")
    params = { "status": ArticleStatus.PUBLISHED.value, "city": city, "state": state}
    try:
        with engine.connect() as conn:
            result = conn.execute(query, params).all()
            return result
