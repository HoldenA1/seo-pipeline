...
def insert_into_db(cursor: sqlite3.Cursor, place: places_v1.Place):
    place_id = place.id
    name = place.display_name.text
    address = place.formatted_address
    rating = place.rating
    rev_count = place.user_rating_count
    summary = place.editorial_summary.text
    website = place.website_uri

    cursor.execute("""
        INSERT OR IGNORE INTO places (id, name, address, rating, reviews_count, editorial_summary, business_site)
        VALUES (?, ?, ?, ?, ?, ?, ?)
    """, (place_id, name, address, rating, rev_count, summary, website))
...
search_text("Korean BBQ", "Berkeley, CA", INCLUDED_FIELDS)